...
const personaCreate: Tool = {
    ...
    params: [
      {
        name: "name",
        type: "string",
        description: "Name for the custom persona",
        required: true,
      },
      {
        name: "role",
  ...
  execute: async (
    ...
    try {
      ...
      const toolScopes =
        args.tool_scopes && typeof args.tool_scopes === "string"
      ...
      return {
        ...
        output: JSON.stringify({
          ...
          message:
            `Custom persona "${persona.name}" created with ID "${persona.id}".`,
        }),
        durationMs: Date.now() - start,
      };
    } catch (error) {
      ...
      if (!activePersona) {
        return {
          ...
          output: JSON.stringify({
            active: null,
            message:
              "No persona is currently active. Using default agent behavior.",
          }),
          durationMs: Date.now() - start,
        };
      }
